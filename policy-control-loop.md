...
*governance-layer design doc. Local. Composes the six roles + an enforcement host's ops into one pipeline.*
*Opened 2026-09-04. ctrl orchestrates · an enforcement host governs · Loomground grounds.*
...
```
  ...
  (4) COMPILE     policy-officer      loomground-governance:loomground / the host's        -> validated .lg twin (express/policy/host)
        |                             policy-ingest op; the host's patch-validate check        applied:false  (a DRAFT — enforces nothing)
        |                             (fail-closed)
        v
  (5) APPLY       policy-officer      the host's patch-apply op                           -> policy on the signed chain (policy_fingerprint)
        |            [RESERVED -> workspace_owner, L4]
        |            *** the reserved human gate — no silent auto-apply ***
        v
  (6) ENFORCE     the gate            the host's signed decision gate reads the applied   -> a Loomground verdict (auto/human/reserved/
        |                             .lg + egress_check (lock-steward) for content at       prohibited) per action (X so both axes hold)
        |                             the boundary
        v
  (7) REBIND      policy-officer      the host's lane-rebind op                           -> every agent's NEXT act checked vs the new policy_fingerprint
        |
        v
  (8) AUDIT       auditor             the host's chain-verify / shadow-scan / discipline  -> integrity + coverage, read-only, folder-scoped
                                       ops
```
...
- Reserved/host: the `.lg` egress obligation, the lock audit path, a real (non-mock) lock backend,
  LG3 MARKERS_DE PR.
- The loop is a COMPOSITION over existing loomground skills + the enforcement host's ops — no
  parallel grounding or gate.
