...
| `governance-layer-toolchain` | Governance-layer toolchain | build/compile/validate the role governance blocks → SKILL.md + `.lg`; mints the `.lg` RVND treats as ground truth; emits for human review, enforces nothing | **L0** | Felix (flxk1) | L0 by construction (not an agent key): `build`/`compile` emit a **human-reviewed diff** (never auto-run in cron/CI or committed unattended); `validate` is **read-only** (the G9 gate). `apply_patch`/auto-run-and-commit-in-CI/hand-editing-generated-`.lg`/emitting-an-unstamped-artefact **prohibited**. Brief: `agents/governance-layer-toolchain.md` | 2026-09-06 |
